# CyberRealistic Pony 提示词存档

机器 A、checkpoint `CyberRealisticPony_V18.0_F16.safetensors`。
832×1216，Steps 30，CFG 5，`dpmpp_2m` + `karras`，CLIP Skip 2。
和 Pony 的差异说明见 [`14-cyber-vs-pony-prompts.md`](14-cyber-vs-pony-prompts.md)。
...
外阴那一段还在，所以衬衫不能盖住胯；不写 `no panties` 会画成整件遮住。

负向：删 `clothes, dressed, bikini, bra, panties, swimsuit`（这串会否掉衣服）。
换成 `fully clothed, pants, jeans, shorts, covered pussy, crotch covered`
（只挡「穿太满、把下体盖住」，不挡衬衫）。

若要下体也遮住，还得删正向外阴整段，本次没动。
